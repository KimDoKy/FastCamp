...
**Nginx** 
웹 서버. 클라이언트로부터의 HTTP요청을 받아 정적인 페이지/파일을 돌려줍니다.

**Django**  
웹 애플리케이션. 웹 요청에 대해 동적데이터를 돌려줍니다.

**uWSGI**  
웹 서버(Nginx)와 웹 애플리케이션(Django)간의 연결을 중계해줍니다.  
(Nginx에서 받은 요청을 Django에서 처리하기 위한 중계인 역할을 해줍니다)
...
**권한 설정 후 aws 에 pem파일을 통해 AWS 접속**(로컬 경로 상관없음)  
...
`l`명령어로 `zsh`이 잘 되어이 있는지 확인해야합니다.
...
### 서버에 환경설정
-
**pyenv 3.5.2 설치** (서버 /srv/에서)

```
pyenv install 3.5.2
```

**가상환경 설정**(서버 /arv/app/에서)  

```
pyenv virtualenv 3.5.2 czarcie
```
> 가상환경을 잘못 설치했을 경우  
> `pyenv uninstall 가상환경명` 으로 삭제하면 된다.
...
```
> `DisallowedHost at /` error는 `ALLOWED_HOSTS` 작업을 안하면 나는 error입니다.
...
> (의존성을 검사하며 설치)  
> 재접속하는데 시간이 좀 걸림.
> error `port 22: Operation timed out` 는 서버가 재가동중이라서 timed out error가 발생함.
> 재접속하면 업데이트 할 내용과 재시작해야한다는 메세지가 없어집니다.
...
> 경로 입력할때는 tap을 이용하여 자동 완성해서 입력하세요.
...
```
> `/etc/nginx/nginx.conf`파일에 어떤 폴더에 있는 설정을 가져와서 실행할 지 적혀있음
...
## IAM Use성 생성
**IAM**(AWS Identity and Access Management) : 사용자를 위해 AWS 리소스에 대한 엑서스를 안전하게 제어할 수 있는 웹서비스입니다. IAM을 사용하여 AWS 리소스를 사용할 수 있는 사람(인증)과 이들이 사용할 수 있는 리소스 및 그 사용 방법(권한부여)을 제어합니다.
>
AWS - IAM - add User
![](./images/iam1.png)
![](./images/iam2.png)
>연습이라서 FullAccess으로 설정한겁니다.
>
![](./images/iam3.png)
![](./images/iam4.png)
> secret_key는 다시 볼 수 없기 때문에 저장해둡니다.
...
#### Instance 주소로 접속하여 Hello,worl 확인.
